...
a new Generated element after this PREVIOUS_NODE. I need to thanks stackoverflow :) */
...
/* In here I summarize all the information that can be different in
 all the little rectangules that happens to show all of my Works (fake ones) :p */
const WORKS_PROPS = [
  {
...
/*
  ALL CONST BELLOWS ARE INSTRUCTIONS TO BUILD ELEMENTS
  WITH ALL PROPERTIES NEEDED, LIKE CLASSES, ID, ALL OF
  THE GOOD STTUFS

  pattern:

  ['element', [...className], 'id', 'type', 'name', 'value'];
*/

/*
  I'll explain this one better so everytime you see another
  similar pattern used in this doc, you'll be able to understand it:

  Here I'm storing information about how many elements a
  const will build and what are the values for the following attributes¹:
  ...
  <section class="recent-works-container flex-container-column" id="portfolio">
  </section>
...
Here's a const that stores the instructions for building
   the card_work_container div element! Yay!!!!
...
/*
  ...
  This is a multi-dimensional array so the format changes a little!

  Correct Function to build all the elements is
  buildAllElements(MultiDimensionalArraysOfInstructions, PostFixIndex,
  WORK_PROPS.techs) - the last two parameters are not mandatory;

  - PostFixIndex = if you have id's that should be named dinamically you
  should add a postFixIndex integer and iterate over it somehow,
   that way the expected ouput should be something like this:

    - 'generic-id' + '-' + PostFixIndex++ <-> 'generic-id-1';

  - WORK_PROPS.techs is required only when you are building the work
...
function buildAllElements(instructions, iterationPosFix, techs) {
  // Cloning the instructions array so I can later splice it without any regrets!
  const copyOfInstructions = [...instructions];

  // Here I'm checking if this argument is valid
  if (iterationPosFix) {
  ...
  /*
    ...
    Here I'm checking if this argument is valid and performing a iteration
    is all of the technologies info stored in the CARD_WORK[someindex
    that's passed in an upper function]['techs'] obj value, so that way
    I can splice some juice info of how many Ul/li are need for this card_work!
...
/*
  ...
  Here I managed to get all elements nested correctly, but a requirement
